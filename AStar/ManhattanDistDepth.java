...
public class ManhattanDistDepth {
	...
	private int NumberMoves, f_limit, minLimit;
	...
	ManhattanDistDepth(String intial, String finalGoal)
	{
	...
	public void search(){
		f_limit = manhattanDistance(current);
		...
		while(!frontier.isEmpty())
		{
			try{
				...
				if(zeroPos != 0 && zeroPos != 1 && zeroPos != 2 && zeroPos != 3){
					...
					if (manhattanDistance(nextChild) < f_limit)
						update(nextChild, current, "U");
					else
						minLimit = Math.min(minLimit, manhattanDistance(nextChild));
				}
